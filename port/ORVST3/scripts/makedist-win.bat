...
if %1 == 1 (echo Making ORVST3 Windows DEMO VERSION distribution ...) else (echo Making ORVST3 Windows FULL VERSION distribution ...)
...
REM - msbuild ORVST3-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild ORVST3.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild ORVST3.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\ORVST3.aaxplugin\Contents\Win32\ORVST3.aaxplugin --out .\build-win\aax\bin\ORVST3.aaxplugin\Contents\Win32\ORVST3.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\ORVST3.aaxplugin\Contents\x64\ORVST3.aaxplugin --out .\build-win\aax\bin\ORVST3.aaxplugin\Contents\x64\ORVST3.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\ORVST3.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\ORVST3.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "ORVST3 Installer" ".\installer\ORVST3 Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\ORVST3 Installer.exe" ".\installer\ORVST3 Demo Installer.exe"
REM -del ".\installer\ORVST3 Installer.exe"
REM -)
